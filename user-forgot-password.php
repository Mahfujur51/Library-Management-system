...
$newpassword=md5($_POST['newpassword']);
  $sql ="SELECT * FROM tbl_student WHERE email='$email' AND mobile='$mobile'";
  $query=mysqli_query($con,$sql);
  $num=mysqli_num_rows($query);
  if ($num>0) {
    $sqlup="UPDATE tbl_student SET password='$newpassword' WHERE mobile='$mobile'";
    $upquery=mysqli_query($con,$sqlup);
    if ($upquery) {
    ...
    echo "<script>alert('Email id AND Mobile no is invalid');</script>"; 
